...
<section id="builder" class="space-y-4">
	<h2 class="text-xs uppercase tracking-widest text-slate-400">Builder</h2>

	<div class="rounded-xl border border-neutral-800 bg-neutral-950 p-4 space-y-5">
		<div class="grid gap-4 md:grid-cols-2">
			<div>
				<p class="text-xs uppercase tracking-wider text-slate-400 mb-2">Time</p>
				<FieldChips bind:spec={minute}
										title={`Minute (${LIMITS.minute.min}-${LIMITS.minute.max})`}
										min={LIMITS.minute.min}
										max={LIMITS.minute.max} />
				<div class="h-3"></div>
				<FieldChips bind:spec={hour}
			...
			<div>
				<p class="text-xs uppercase tracking-wider text-slate-400 mb-2">Date</p>
				<FieldChips bind:spec={dom}
										title={`Day of Month (${LIMITS.dom.min}-${LIMITS.dom.max})`}
										min={LIMITS.dom.min}
										max={LIMITS.dom.max} />
				<div class="h-3"></div>
				<FieldChips bind:spec={month}
										title={`Month (${LIMITS.month.min}-${LIMITS.month.max})`}
										min={LIMITS.month.min}
										max={LIMITS.month.max}
										labels={MONTH_LABELS} />
				<div class="h-3"></div>
				<FieldChips bind:spec={dow}
		...
		<div class="space-y-3">
			<p class="text-xs uppercase tracking-wider text-slate-400">Output</p>
			<div class="flex items-center justify-between gap-2 flex-wrap">
				<div class="flex items-center gap-2 flex-wrap">
					<button type="button" aria-label="Select cron text" title="Click to select"
									class="rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-2 font-mono text-sm text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-950 transition-colors cursor-pointer"
									onclick={selectText}>{cronOut}</button>
					<CopyButton text={cronOut} label="Copy" title="Copy cron expression" />
				</div>
				<button type="button"
								class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-neutral-800 bg-neutral-900 px-3 py-2 text-sm font-semibold hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-950 transition-colors"
								title="Reset all fields to Every" aria-label="Reset to Every" onclick={resetAll}>
					Reset
				</button>
			</div>
